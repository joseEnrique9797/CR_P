...
class purchaseConsolidateLine(models.Model):
    ...
    qty_received_actual = fields.Float('Recibido en esta consolidación', compute = 'set_qty_received_actual')
    qty_received = fields.Float('Total recibido', related = 'purchase_line_data_id.qty_received')
    ...
    def set_qty_received_actual(self):
        for rec in self:
            lines = rec.env['stock.move'].search([
                ('consolidate_id','=',rec.consolidate_id.id),
                ('product_id','=',rec.product_id.id),
            ])

            rec.qty_received_actual = 0
            rec.qty_received_actual = sum(l.quantity_done for l in lines)
    ...
    def action_desp_set(self):
        quant = self.qty - self.qty_transito
        if self.qty_received_actual >= self.qty_transito :
